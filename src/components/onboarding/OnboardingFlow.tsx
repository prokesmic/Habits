        ...
        >
          {currentStep === 1 && (
            <div className="space-y-6 text-center">
              ...
              <h2 className="text-2xl font-bold">Welcome to Habit Tracker</h2>
              <p className="text-gray-600">Build habits that stick with real accountability</p>
              <div className="space-y-3">
                ...
                <a href="/auth/sign-in" className="block text-sm text-gray-700 underline">
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-600">Choose your primary goal</p>
      </div>
      <div className="space-y-3">
        {goals.map((goal) => (
          <button
...
function FirstHabitStep({
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="mb-2 text-2xl font-bold">Create your first habit</h2>
        <p className="text-gray-600">This is what you'll track daily</p>
      </div>
      {habitSuggestions.length > 0 && (
        <div>
          <div className="mb-2 text-sm font-medium">Quick picks:</div>
      ...
      <div>
        <label className="mb-2 block text-sm font-medium">Choose an icon</label>
...
function StakeStep({
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="mb-2 text-2xl font-bold">Put money on it? (Optional)</h2>
        <p className="text-gray-600">You're 4x more likely to succeed when money is on the line.</p>
      </div>
      <div className="grid grid-cols-4 gap-2">
        {[5, 10, 25].map((v) => (
          <button
            key={v}
            onClick={() => setAmount(v)}
            className={`rounded-lg border px-3 py-2 text-sm ${amount === v ? "border-violet-500 bg-violet-50" : "border-gray-200"}`}
          >
            ${v}
          </button>
        ))}
        <button onClick={() => setAmount(undefined)} className="rounded-lg border px-3 py-2 text-sm">
          Skip
        </button>
      </div>
...
function SquadStep({ onComplete, onSkip }: { onComplete: (squadId: string) => void; onSkip: () => void }) {
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="mb-2 text-2xl font-bold">Find your accountability crew</h2>
        <p className="text-gray-600">Users in squads are 23% more likely to succeed!</p>
      </div>
      <div className="space-y-3">
        {popular.map((s) => (
          <div key={s.id} className="rounded-xl border p-4">
            <div className="mb-1 font-semibold">{s.name}</div>
            <div className="mb-3 text-sm text-gray-600">{s.meta}</div>
            <button onClick={() => onComplete(s.id)} className="rounded-lg border px-3 py-2 text-sm">
              Join
            </button>
          </div>
        ))}
      </div>
      <button onClick={onSkip} className="w-full rounded-lg border-2 border-gray-300 py-3 font-semibold hover:bg-gray-50">
        Skip for now
      </button>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <h2 className="mb-2 text-2xl font-bold">Never miss a day</h2>
        <p className="text-gray-600">Daily reminders help you stay on track</p>
      </div>
      ...
      <div>
        ...
        <input type="time" value={time} onChange={(e) => setTime(e.target.value)} className="w-full rounded-lg border px-4 py-3 text-lg" />
      </div>
