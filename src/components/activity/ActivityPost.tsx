    ...
    <div className="mt-4">
      ...
      <div className="mt-2 flex items-center gap-3 text-sm font-semibold text-slate-600">
        {actions.includes("like") && (
          <button className="rounded-full px-2 py-1 transition hover:bg-slate-100">Like</button>
        )}
        {actions.includes("comment") && (
          <button className="rounded-full px-2 py-1 transition hover:bg-slate-100">Comment</button>
        )}
        {actions.includes("share") && (
          <button className="rounded-full px-2 py-1 transition hover:bg-slate-100">Share</button>
        )}
        {actions.includes("reply") && (
          <button className="rounded-full px-2 py-1 transition hover:bg-slate-100">Reply</button>
        )}
        {actions.includes("react") && (
          <button className="rounded-full px-2 py-1 transition hover:bg-slate-100">React</button>
        )}
      </div>
    </div>
  );
