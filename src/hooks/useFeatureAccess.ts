...
import { subscriptionPlans, type SubscriptionPlan } from "@/lib/billing/plans";
...
export const useFeatureAccess = () => {
  ...
  const canAccess = (feature: keyof SubscriptionPlan["features"]) => {
    if (!plan) return false;
    return Boolean(plan.features[feature]);
  };

  const checkLimit = (limitType: keyof SubscriptionPlan["limits"], current: number) => {
    if (!plan) return false;
  ...
  const requiresPremium = (feature: keyof SubscriptionPlan["features"]) => {
